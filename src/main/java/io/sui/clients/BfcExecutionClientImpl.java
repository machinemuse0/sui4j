 ...
 * @since 2022.11
 */
public class BfcExecutionClientImpl implements ExecutionClient {
   ...
   */
  public BfcExecutionClientImpl(JsonRpcClientProvider jsonRpcClientProvider) {
  ...
  public CompletableFuture<TransactionBlockEffects> dryRunTransaction(String txBytes) {
    final JsonRpc20Request request =
        this.jsonRpcClientProvider.createJsonRpc20Request(
            "bfc_dryRunTransaction", Lists.newArrayList(txBytes));
    return this.jsonRpcClientProvider.callAndUnwrapResponse(
        "/bfc_dryRunTransaction", request, new TypeToken<TransactionBlockEffects>() {}.getType());
  }
